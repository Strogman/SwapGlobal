...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "subterraneans") and npcHandler.topic[cid] == 1 then
		if player:getStorageValue(Storage.DangerousDepths.Dwarves.Subterranean) == 2 and player:getStorageValue(Storage.DangerousDepths.Dwarves.timeTaskSubterranean) < os.time() then
	...
	if msgcontains(msg, "home") and npcHandler.topic[cid] == 1 then
		if player:getStorageValue(Storage.DangerousDepths.Dwarves.Home) == 2 and player:getStorageValue(Storage.DangerousDepths.Dwarves.timeTaskHome) < os.time() then
			npcHandler:say({"I don't need your help for now. Come back later."}, cid)
			playerTopic[cid] = 1
			npcHandler.topic[cid] = 1
		end
