...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "diremaws") and npcHandler.topic[cid] == 1 then
		if player:getStorageValue(Storage.DangerousDepths.Scouts.Diremaw ) == 2 and player:getStorageValue(Storage.DangerousDepths.Scouts.timeTaskDiremaws) < os.time() then -- Ainda não se passaram as 20h
			npcHandler:say({"I don't need your help for now. Come back later."}, cid)
			playerTopic[cid] = 1
			npcHandler.topic[cid] = 1
		end
		if player:getStorageValue(Storage.DangerousDepths.Scouts.Diremaw) == 2 and player:getStorageValue(Storage.DangerousDepths.Scouts.timeTaskDiremaws) > os.time() then -- Vai fazer a missão após 20h
		...
		elseif player:getStorageValue(Storage.DangerousDepths.Scouts.Diremaw) == 1 and player:getStorageValue(Storage.DangerousDepths.Scouts.DiremawsCount) >= 20 then -- Não possuía a missão, agora possui!
			...
			player:setStorageValue(Storage.DangerousDepths.Scouts.timeTaskDiremaws, os.time() + tempo)
	...
	if msgcontains(msg, "growth") and npcHandler.topic[cid] == 1 then
		if player:getStorageValue(Storage.DangerousDepths.Scouts.Growth ) == 2 and player:getStorageValue(Storage.DangerousDepths.Scouts.timeTaskGrowth) < os.time() then -- Ainda não se passaram as 20h
			npcHandler:say({"I don't need your help for now. Come back later."}, cid)
			playerTopic[cid] = 1
			npcHandler.topic[cid] = 1
		end
		if player:getStorageValue(Storage.DangerousDepths.Scouts.Growth) == 2 and player:getStorageValue(Storage.DangerousDepths.Scouts.timeTaskGrowth) > os.time() then -- Vai fazer a missão após 20h
