...
function onUse(player, item, fromPosition, itemEx, toPosition)
	...
	if doCheckBossRoom(player:getId(), "The First Dragon", frompos, topos) then
				...
				for _z = fromPos.z, toPos.z do
					...
					if tile then
						...
						for _, pid in pairs(plist) do
							...
							if tmpPlayer and tmpPlayer:getStorageValue(Storage.TheFirstDragon.theFirstDragonTime) < os.time() then --tmpPlayer:getStorageValue(Storage.TheFirstDragon.portaoFinal) >= 15 then
		...
		for _, plid in pairs(players)do
			...
			plid:setStorageValue(Storage.TheFirstDragon.theFirstDragonTime, os.time() + 20*60*60) 
			-- if(count % 3 == 0)then
				-- passage = passage + 1	
			-- end
