...
function onUsePick(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if target.itemid == 30723 then -- Gold
		if player:getStorageValue(Storage.CurseSpreads.rewardgold) < 1 then
			player:setStorageValue(Storage.CurseSpreads.rewardgold, 1)
	...
	elseif target.itemid == 30724 then -- Silver
		if player:getStorageValue(Storage.CurseSpreads.rewardsilver) < 1 then
			player:setStorageValue(Storage.CurseSpreads.rewardsilver, 1)
	...
	-- shiny stone refining
		...
		elseif targetId == 6299 and (target:hasAttribute(ITEM_ATTRIBUTE_UNIQUEID) or target:hasAttribute(ITEM_ATTRIBUTE_ACTIONID)) then
...
function onUseSpoon(player, item, fromPosition, target, toPosition, isHotkey)
	...
	elseif targetId == 8573 or targetId == 388 then
		if player:getStorageValue(Storage.WhatAFoolishQuest.Questline) >= 21 and player:getStorageValue(Storage.WhatAFoolishQuest.InflammableSulphur) ~= 1 then
