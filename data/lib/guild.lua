...
function Guild.registerAction(self, pid, amount, type)
    ...
    db.query('INSERT INTO `guild_actions_h` (`guild_id`, `player_id`, `value`, `date`, `type`) VALUES ('..self:getId()..','..p_id..','..newAmount..','..os.time()..', '..type..')')
end
	...
	if player then
		if not toGuild then
			...
			if fromBalance < amount then
				...
				db.query('INSERT INTO `guild_transfer_h` (`player_id`, `from_guild_id`, `to_guild_id`, `value`, `date`) VALUES ('..player:getGuid()..', '..self:getId()..','..toGuild:getId()..','..amount..','..os.time()..')')
			end
		end
