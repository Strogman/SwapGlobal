...
function Player.teleportToWar(self)
	...
	if city > 0 then
		...
		if PRIVATEWAR_BATTLEFIELD[city].started and (time - os.time()) < 10 then
			return false
		end
		...
		if not PRIVATEWAR_BATTLEFIELD[city].started then
			...
			PRIVATEWAR_BATTLEFIELD[city].endtime = (PRIVATEWAR_BATTLEFIELD[city].time * 60) + os.time()
...
function Game.startWar(attackerId, defenseId, data)
	...
	if PRIVATEWAR_BATTLEFIELD[data.city] and PRIVATEWAR_BATTLEFIELD[data.city].started and PRIVATEWAR_BATTLEFIELD[data.city].endtime >= os.time() then
		return false
	end
...
function Game.checkWar(attackerId, defenseId, city)
	...
	if PRIVATEWAR_BATTLEFIELD[city].started then
		if PRIVATEWAR_BATTLEFIELD[city].endtime < os.time() then
