local CHANNEL_ENGLISH = 4
local storage = STORAGE_MUTED_ON_CHANNELS + CHANNEL_ENGLISH
...
function onSpeak(player, type, message)
	...
	if player:getStorageValue(storage) > os.time() then
        player:sendCancelMessage("You are muted from the english channel for using it inappropriately.")
        return false
    end
