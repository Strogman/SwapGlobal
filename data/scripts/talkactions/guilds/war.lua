...
function talkaction.onSay(player, word, param)
	...
	if Game.getStorageValue(GlobalStorage.WarDelay1) >= os.time() then
		player:sendCancelMessage(string.format("You are exhausted"))
		return false
	end

	if player:getStorageValue(Storage.WarDelay1) >= os.time() then
		player:sendCancelMessage(string.format("You are exhausted"))
		return false
	end
	player:setStorageValue(Storage.WarDelay1, os.time() + 10)
	Game.setStorageValue(GlobalStorage.WarDelay1, os.time() + 3)
	...
	elseif split[1] == "accept" then
		...
		local queryText = string.format("UPDATE `guild_wars` SET `status` = 1, `started` = %d WHERE `id` = %d", os.time(), warid)
		db.asyncQuery(queryText)
	...
	elseif split[1] == "cancel" then
		...
		local queryText = string.format("UPDATE `guild_wars` SET `status` = 3, `ended` = %d WHERE `id` = %d", os.time(), warid)
		db.asyncQuery(queryText)
	...
	elseif split[1] == "reject" then
		...
		local queryText = string.format("UPDATE `guild_wars` SET `status` = 2, `ended` = %d WHERE `id` = %d", os.time(), warid)
		db.asyncQuery(queryText)
