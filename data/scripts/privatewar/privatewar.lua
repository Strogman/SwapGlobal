...
function talk.onSay(player, word, param)
	...
	if split[1] == "invite" then
		...
		if player:getStorageValue(Storage.WarDelay) >= os.time() then
			player:sendCancelMessage("You are exhausted.")
			return false
		end
		local StorageGlobal = Game.getStorageValue(GlobalStorage.WarDelay) or -1
		if StorageGlobal >= os.time() then
			player:sendCancelMessage("You are exhausted.")
			return false
		end

		-- evitar varias queries ao mesmo tempo no servidor
		Game.setStorageValue(GlobalStorage.WarDelay, os.time() + 3)
		-- delay do jogador
		player:setStorageValue(Storage.WarDelay, os.time() + 10)
